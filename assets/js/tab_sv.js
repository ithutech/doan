...
function infoCompanySV_() {
  ...
  $.getJSON(url, function(data) {
    ...
    strText += '</div>';
    var jsEx = '$("#submitChangeINFC").click(function(e) { var formInfo = document.getElementById("form_update_cp"); var em = $("input[name =\"manager_email\"]").val(); var ws = $("input[name =\"website\"]").val(); var mfn = $("input[name =\"manager_fullname\"]").val(); var mphn = $("input[name =\"manager_phonenumber\"]").val(); if(em == "" || ws == "" || mfn == "" || mphn == "") return alert("Vui lòng nhập đầy đủ thông tin | Nếu chưa có vui lòng ghi 'Chưa'"); var formData = new FormData(formInfo); var url = APIurl + "&action=stUpdate_C_INFO"; document.querySelector(".form-input-absoluted .submit-form-status").classList.add("disabled"); $.ajax({ type: "post", url: url, data: formData, contentType: false, processData: false, success: function(data) { document.querySelector(".form-input-absoluted .submit-form-status").classList.remove("disabled"); return alert(data.result); } }); });';
